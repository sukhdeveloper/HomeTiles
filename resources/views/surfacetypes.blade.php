...
function enableSelectedTypes() {
    ...
    if (checkSelectedTypes()) {
        ...
        window.$('#typesForm').attr('action', '/surfacetypes/enable');
        window.$('#typesFormInput').val(JSON.stringify(checkedTypes));

        window.$('#confirmDialogHeader').text('Confirm enabling Surface Types');
        window.$('#confirmDialogText').text('Please confirm enabling selected ' + checkedTypes.length + ' Surface Types.');
        window.$('#confirmDialogSubmit').text('Enable Surface Types');
...
function disableSelectedTypes() {
    ...
    if (checkSelectedTypes()) {
        var checkedTypes = getCheckedTypesArray();
        window.$('#typesForm').attr('action', '/surfacetypes/disable');
        window.$('#typesFormInput').val(JSON.stringify(checkedTypes));

        window.$('#confirmDialogHeader').text('Confirm disabling Surface Types');
        window.$('#confirmDialogText').text('Please confirm disabling selected ' + checkedTypes.length + ' Surface Types.');
        window.$('#confirmDialogSubmit').text('Disable Surface Types');
        window.$('#confirmDialog').modal('show');
    } else {
        window.$('#warningAlertBox').fadeIn();
    }
...
function deleteSelectedTypes() {
    ...
    if (checkSelectedTypes()) {
        var checkedTypes = getCheckedTypesArray();
        window.$('#typesForm').attr('action', '/surfacetypes/delete');
        window.$('#typesFormInput').val(JSON.stringify(checkedTypes));

        window.$('#confirmDialogHeader').text('Confirm removing Surface Types');
        window.$('#confirmDialogText').text('Please confirm removing selected ' + checkedTypes.length + ' Surface Types.');
        window.$('#confirmDialogSubmit').text('Remove Surface Types');
        window.$('#confirmDialog').modal('show');
    } else {
        window.$('#warningAlertBox').fadeIn();
    }
...
function deleteType(id) {
    ...
    window.$('#confirmDialogHeader').text('Confirm removing Surface Type');
    window.$('#confirmDialogText').text('Please confirm removing Surface Type.');
    window.$('#confirmDialogSubmit').text('Remove Surface Type');
    window.$('#confirmDialog').modal('show');
}
</script>
...
<div id="addTypeFormBlock" class="panel-body" style="display: none;">
  <form action="/surfacetype/add" method="POST" class="form-horizontal">
    {{ csrf_field() }}

    <div class="form-group">
      <label for="form-type-name" class="col-sm-3 control-label">Surface Type name</label>
      <div class="col-sm-6">
        <input type="text" name="name" id="form-type-name" class="form-control" placeholder="Surface Type name" maxlength="100">
      </div>
    </div>
    ...
    <div class="form-group">
      <div class="col-sm-offset-3 col-sm-6">
        <span class="pull-right">
          <button type="submit" class="btn btn-primary">Add Surface Type</button>
...
<div id="updateTypeFormBlock" class="panel-body" style="display: none;">
  <form action="/surfacetype/update" method="POST" class="form-horizontal" id="updateTypeForm">
    {{ csrf_field() }}
    ...
    <div class="form-group">
      <label for="form-update-type-name" class="col-sm-3 control-label">Surface Type name</label>
      <div class="col-sm-6">
        <input type="text" name="name" id="form-update-type-name" class="form-control" placeholder="Surface Type name" maxlength="100">
      </div>
    </div>

    <div class="form-group">
      <div class="col-sm-offset-3 col-sm-6">
        <button type="button" class="btn btn-default" onclick="deleteType(window.$('#form-update-type-id').val());" title="Remove Surface Type"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></button>
        <span class="pull-right">
          <button type="submit" class="btn btn-primary">Update Surface Type</button>
...
<div class="panel panel-default">
  <div class="dropdown pull-right">
    <button class="btn btn-default btn-sm" onclick="window.$('#addTypeFormBlock').toggle('fast'); window.$('#updateTypeFormBlock').hide('fast');">Add Surface Type</button>
  ...
  <h3 class="panel-heading">Surface Types List</h3>
